 ...
 * <p>
 * This component belongs to <b>Layer 2 (Cognition / Motivation)</b> of the MiRoVA architecture. It handles "Get out of the way"
 * and "Stay out of the way" behaviors resulting from social pressure.
 * </p>
 * <p>
...
public class SocialInteractionsIncentives extends DesireIncentive
{
     ...
     * <p>
     * Applicable only if the ego vehicle's speed is above a certain threshold (e.g., 20 m/s), as social interaction pressures
     * are typically negligible at low speeds.
     * </p>
    ...
    public Desire computeDesire() throws ParameterException, OperationalPlanException, GtuException, NetworkException
    {
        ...
        try
        {
            if (getInfrastructurePerception().getLegalLaneChangePossibility(RelativeLane.CURRENT, LateralDirectionality.LEFT)
                    .gt(getParameters().getParameter(ParameterTypes.LOOKAHEAD)) && rhoPotentialFollower != null
                    && rhoEgoPotential != null && rhoPotentialFollower * socioSpeedSensitivity > rhoEgoPotential)
            {
    ...
    /**
     * Returns a normalized social pressure, equal to (vDesired - vLead) / vGain. * @param followerDesiredSpeed desired speed of
     * the follower
     ...
     */
    static double socialPressure(final Speed followerDesiredSpeed, final Speed leaderSpeed, final Speed vGain,
            final Length headway, final Length followerLookahead)
    {
