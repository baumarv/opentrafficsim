 ...
 * <p>
 * This class forms the default logic for <b>Layer 3 (Decision / Strategy)</b> in the MiRoVA architecture. It implements a
 * lightweight deterministic rule set to choose the best maneuver to execute:
 * <ul>
 * <li>Only patterns for which <b>all required contextual conditions</b> are satisfied ({@link ManeuverPattern#checkContext()})
 * and physically feasible ({@link ManeuverPattern#checkAbility()}) are considered.</li>
 * <li>Among those, the pattern with the <b>largest number of required context keys</b> (i.e., the most specific pattern) is
 * preferred.</li>
 * <li>If two patterns have the same specificity, a predefined <b>context-type priority hierarchy</b> is applied to resolve
 * ties.</li>
 * </ul>
 * </p>
 * <p>
...
public class DeterministicSpecificityStrategy implements PatternSelectionStrategy
{
     ...
     * <p>
     * The method iterates over all provided patterns. Each pattern is tested for both contextual and physical validity. Only
     * fully applicable patterns are considered. The pattern with the highest number of required context categories is selected
     ...
     */
    @Override
    public ManeuverPattern select(final ArrayList<ManeuverPattern> patterns) throws ParameterException
    {
     ...
     * <p>
     * Tie-breaking is based on a predefined context-type priority hierarchy. Context categories that appear later in the list
     * are considered more specific (and therefore higher priority) than those appearing earlier.
     * </p>
    ...
    /**
     * Computes a numerical specificity score based on the intersection between a pattern's required context keys and a given
     * context-type priority list.
     * <p>
     * Contexts appearing later in the list are given higher weight to reflect greater situational specificity.
     * </p>
