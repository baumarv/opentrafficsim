 ...
 * <p>
 * This pattern activates when the average speed on any adjacent lane falls below the congestion threshold ({@code VCONG}) while
 * the ego vehicle is still at free-flow speed. It applies a soft deceleration bounded by
 * {@link MirovaParameters#preemptiveCooperativeDeceleration} to gradually adapt speed without abrupt braking, improving
    ...
    public static class AdjacentCongestionState extends ActionState
    {
        ...
        public SimpleOperationalPlan abort() throws ParameterException, GtuException, NetworkException
        {
            ...
            for (LateralDirectionality dir : this.maneuverPattern.congestedLanes)
            {
                ...
                else if (dir == LateralDirectionality.RIGHT && macro.getAverageSpeed(RelativeLane.RIGHT).lt(vCong))
                {
                    return null;
                }
            }
