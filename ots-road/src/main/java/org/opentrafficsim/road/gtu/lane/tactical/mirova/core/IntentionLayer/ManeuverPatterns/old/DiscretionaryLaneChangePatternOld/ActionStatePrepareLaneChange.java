package org.opentrafficsim.road.gtu.lane.tactical.mirova.core.IntentionLayer.ManeuverPatterns.old.DiscretionaryLaneChangePatternOld;
 ...
 * <p>
 * First phase of the lane-change maneuver. The vehicle maintains normal car-following behavior while continuously checking
 * whether a lane change in the desired direction becomes feasible.
 * </p>
 * <p>
 * <b>Transitions:</b>
 * </p>
...
public class ActionStatePrepareLaneChange extends ActionState
{
     ...
     * <p>
     * During the preparation phase, no lateral motion is yet performed. The vehicle adapts its longitudinal acceleration using
     * the MIROVA car-following controller.
     * </p>
    ...
    public SimpleOperationalPlan executeControl() throws ParameterException, GtuException, NetworkException
    {
        ...
        SimpleOperationalPlan plan =
                new SimpleOperationalPlan(acceleration, this.vehicle.getGtu().getParameters().getParameter(ParameterTypes.DT));
     ...
     * <p>
     * The state transitions to {@code ActionStatePerformLaneChange} if all feasibility conditions are met (ego and follower
     * decelerations above threshold, lane not ending too soon).
     * </p>
     * @return
     * @throws NetworkException
     * @throws GtuException
            ...
            throws ParameterException, NullPointerException, IllegalArgumentException, GtuException, NetworkException
    {
        if (checkFeasibility())
        {
            ActionState nextState = new ActionStatePerformLaneChange(this.maneuverPattern, this.direction);
     ...
     * <p>
     * The state aborts if the feasibility check fails or the lane-change desire drops significantly below the discretionary
     * threshold.
     * </p>
     * @return
     */
    ...
    {
        if (!checkFeasibility() || this.vehicle.getDesire() < this.vehicle.getDFree() - DESIRE_HYSTERESIS)
        {
     ...
     * <p>
     * The check is based on the NeighborsContext and current deceleration capabilities of both ego and follower vehicles on the
     * target lane.
     * </p>
    ...
    private boolean checkFeasibility() throws ParameterException
    {
        ...
        return egoDecel.gt(bDes) && follDecel.gt(bDes) && rearHeadway.gt(desiredRearHeadway)
