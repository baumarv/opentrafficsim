...
public class InfrastructureContext extends ContextCategory implements UpdatableContext
{
    ...
    /** Cache key prefix for anticipated speed per relative lane. */
    private static final String ANTICIPATED_SPEED_PREFIX = "anticipatedSpeed_";
    ...
    /**
     * Returns the anticipated speed on the given relative lane, blending the free desired speed with the speeds of all visible
     * leaders weighted by their distance.
     * <p>
     * For each leader at headway {@code s}, its effective contribution is:
     * 
     * <pre>
     *   ṽ_lead(s) = (1 − s/x0) · v_lead + (s/x0) · v_des,  for s &lt; x0
     *   ṽ_lead(s) = v_des,                                    for s ≥ x0
     * </pre>
     * 
     * The anticipated speed is the minimum of {@code v_des} and all per-leader values, so that a slow nearby leader reduces the
     * anticipation speed while a distant leader has no effect.
     * </p>
     * <p>
     * The result is lazily computed and cached per relative lane per simulation tick.
     * </p>
     * @param lane the relative lane to evaluate (e.g., CURRENT, LEFT, RIGHT)
     * @return anticipated speed on the given lane [m/s], at most the vehicle's own desired speed
     */
    public Speed getAnticipatedSpeed(final RelativeLane lane)
    {
        ...
        Speed result = computeAnticipatedSpeed(lane);
        cacheValue(key, result, true);
        return result;
    }

    /**
     * Computes the anticipated speed on the given relative lane.
     * <p>
     * Iterates over all visible leaders on the lane via {@link NeighborsContext} and applies the distance-weighted blending
     * formula. Leaders beyond the anticipation distance {@code x0} are fully ignored (contribute {@code v_des}).
     * </p>
     * @param lane the relative lane to evaluate
     * @return anticipated speed [m/s]
     */
    private Speed computeAnticipatedSpeed(final RelativeLane lane)
    {
        try
        {
            Speed vDes = this.vehicle.getGtu().getDesiredSpeed();
            double vDesSI = vDes.si;
            double x0SI = this.vehicle.getParameters().getParameter(ParameterTypes.LOOKAHEAD).si;

            LateralDirectionality dir;
            if (lane.equals(RelativeLane.LEFT))
            {
                dir = LateralDirectionality.LEFT;
            }
            else if (lane.equals(RelativeLane.RIGHT))
            {
                dir = LateralDirectionality.RIGHT;
            }
            else
            {
                dir = LateralDirectionality.NONE;
            }
            ...
            double anticipatedSI = vDesSI;
            for (HeadwayGtu leader : leaders)
            {
                double sSI = leader.getDistance().si;
                double vLeadSI = leader.getSpeed().si;
                double w = Math.max(0.0, 1.0 - sSI / x0SI); // weight goes from 1 at s=0 to 0 at s=x0
                double vWeighted = w * vLeadSI + (1.0 - w) * vDesSI;
                anticipatedSI = Math.min(anticipatedSI, vWeighted);
            }

            return Speed.instantiateSI(Math.max(0.0, anticipatedSI));
        }
        catch (Exception e)
        {
