 ...
 * the follower on the left adjacent lane would experience if the ego vehicle merged into that lane, as evaluated by the
...
public class ExtendedDataFollowerDecelLeft extends ExtendedDataAcceleration<GtuData>
{
    ...
    {
        super("FollowerDecelLeft",
                "Resulting deceleration for follower on adjacent lane in case of lane change to the left [m/s²]");
    }
    ...
    public FloatAcceleration getValue(final GtuData gtu)
    {
        if (gtu instanceof GtuDataRoad road)
        {
            ...
            {
                Acceleration decel = p.getContext(NeighborsContext.class).getCachedValue(NeighborsContext.FOLLOWER_DECEL_LEFT,
                        Acceleration.class);
