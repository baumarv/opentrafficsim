 ...
 * <p>
 * Represents the active execution phase of a lane-change maneuver. The ego vehicle transitions laterally to the target lane
 * while maintaining longitudinal safety using a simplified Two-Leader-Car-Following logic.
 * </p>
 * <p>
 * <b>Transitions:</b>
 * </p>
...
public class ActionStatePerformLaneChange extends ActionState
{
    ...
    public SimpleOperationalPlan executeControl() throws ParameterException, GtuException, NetworkException
    {
        ...
        if (this.vehicle.getGtu().getLane() == this.originLane)
        {
            ...
            {
                Acceleration aTarget = CarFollowingUtil.followSingleLeader(this.vehicle.getCarFollowingModel(), params,
                        egoSpeed, infraCtx.getCurrentSpeedLimit(), targetLeader.getDistance(), targetLeader.getSpeed());
                minAcc = Acceleration.min(minAcc, aTarget);
            }
        }

        SimpleOperationalPlan plan = new SimpleOperationalPlan(minAcc, params.getParameter(ParameterTypes.DT), this.direction);
