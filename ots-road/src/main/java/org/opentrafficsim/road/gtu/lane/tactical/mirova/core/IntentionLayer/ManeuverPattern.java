 ...
 * <p>
 * A {@code ManeuverPattern} represents a structured tactical or cooperative behavior schema (e.g., lane change, merge,
 * cooperative yield). It embodies the procedural knowledge mapped to <b>Layer 4 (Procedure & Action)</b> of the MiRoVA
 * architecture. Each ManeuverPattern acts as a Finite State Machine (FSM) orchestrating various {@link ActionState}s.
 * </p>
 * <p>
...
public abstract class ManeuverPattern
{
    ...
    /**
     * Checks whether the current contextual situation permits this maneuver pattern (e.g., road type, congestion state,
     * automation level, etc.).
     * <p>
     * Implementations typically query the {@code VehicleContextManager} or similar global context data structures.
     * </p>
    ...
    /**
     * Verifies whether the maneuver can currently be executed from a physical and safety perspective (e.g., gap availability,
     * rule compliance, relative speeds).
     ...
     * pattern's internal state machine.
    ...
    /**
     * Returns the pattern-specific timestep. If not explicitly set, this defaults to the global simulation timestep set in the
     * vehicle parameters.
