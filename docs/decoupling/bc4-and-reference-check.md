...
| 12 | Reference §8: `v_gain` = 15 km/h car, 30 truck | **RESOLVED by decision.** The model now carries 15 / 30 km/h; the published 15 / 30 m/s survives as the `legacy` variant and the tag `campaign-final-v1` (2026-09-16: `published-model` deleted, it reproduced neither campaign). | See the history below. |
...
| 19 | Implicit everywhere: a per-tick cache keyed by a lateral direction is assumed to distinguish the directions it is asked about | **Six direction-keyed caches use `dir == LEFT ? ... : ...`, which sends `NONE` to the RIGHT entry.** One pair (the ego and follower deceleration thresholds) is a real collision and sits behind **BC-12**. Of the other four, `getEgoDeceleration`, `getFollowerDeceleration` and `getIfLaneChangePossible` are **measured** never to receive NONE (0 calls on both production cells); `getParallelMerge` is unreachable by **reasoning** -- its only caller loops over an explicit `{LEFT, RIGHT}` array; and `getIfLaneAvailable` **is** reached by NONE, 16 997 and 15 177 times, and is **measured** inert for two independent reasons (NONE is always served an entry RIGHT wrote first, so it never writes; and the served value equalled NONE's own in all 32 174 cross-reads). | This is a property of the key convention, not four separate cases: a change to `dominantDirection()`, to `getTargetDirection()`, or to any caller that starts passing NONE makes five latent aliases live at once. Pointers sit at each getter and at `dominantDirection()`. Full numbers and verdicts in [`cache-audit.md`](cache-audit.md) §2. |
| 20 | Implicit: a cache key that names a lane identifies it uniquely | **`InfrastructureContext.getLaneAverageSpeed` keys the lane by `lane.getId()` alone**, where the rest of the tree identifies a lane as `link.getId() + "/" + lane.getId()`. All five value-bearing arguments are otherwise in the key. | Not a collision on this network, and not observed -- but uniqueness is then an assumption about the *network*, not a property of the key. It may hold on Freiburg-Nord and need not hold elsewhere, so it travels with the model. Reasoned, not measured. |
...
1. **The published results are preserved.** They were produced at 15 / 30 m/s. The tag
   ...
   *Correction 2026-09-16: `published-model` reproduced neither campaign and is deleted; `final_v1` ran at
   `fbce85dbe`, now tag `campaign-final-v1`, and `legacy` carries `final_v1`'s parameters on the current model
   (`docs/fork-merge-plan.md`, section D).*
...
published run contained. *(2026-09-16: that tag sat at `9eb7ab856`, after eight model fixes `final_v1` did not
have, and is deleted; this section's registration list is unaffected by them — none of the eight registers or
removes a pattern or incentive.)*
