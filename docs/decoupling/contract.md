...
| BC-4 — follower desired speed estimated from observation | **yes, but inert in OTS** | §3: the neighbour's desired speed is not in `PerceivedVehicle`. **Its only consumer, `SocialInteractionsIncentives`, has never been registered**, so the switch changes nothing in any OTS run and `coreset` differs from `reference` by five switches, not six. The core still omits the field — it is not observable — but the campaign cannot measure the substitution. See [`bc4-and-reference-check.md`](bc4-and-reference-check.md) §5. |
...
| BC-10 — induced deceleration cached per quantity | **yes** | Structural, like BC-2 and BC-6: the core has no induced-deceleration memo at all. `egoDeceleration`, `followerDeceleration` and `inducedDeceleration` are three functions, each computing its quantity where it is needed, so there is no key to share. A per-tick memo keyed by the perceived vehicle is a host artefact under ADR-015 and does not enter the port. In OTS the two quantities share the key `inducedDecel_<id>`, and the merge router's follower branch decides on whichever filled it first — measured at 28 of 281 evaluations. Without this switch in `coreset` the gates would compare the core against a Java model doing something else. |
| BC-13 — lane-change desire capped at 1 above, open below | **no** | The core reproduces the uncapped behaviour, and does so faithfully rather than by oversight: `Desire.kt` takes `left`/`right` verbatim with no `require`, `coerce` or `min`; `add` and `scale` are plain arithmetic; `combine` does not clamp; `magnitude()` floors at 0 with no ceiling; and `Incentives.kt` ports the unbounded `damping * Δv / speedGain` exactly, its only bound being the Java one-sided `min(Δv, 0)` to the right above `vCong`. The core's own KDoc asserts "two scalars in [−1, 1]" while the arithmetic does not enforce it — the same contradiction as the Java model and the specification. So `coreset` must keep the uncapped desire, or it would stop being what the core embodies. Adopting BC-13 later is a one-line change in `Desire.kt`'s constructor, but it needs the modelling decision first, and the core's divergence from OTS's own capping `Desire` record is currently recorded nowhere. |
| BC-12 — deceleration-threshold cache keys cover NONE | **yes** | Structural, like BC-10, and for the same reason one step further: the core has no memo on these two quantities at all. `egoDecelerationThreshold(desire)` and `followerDecelerationThreshold(desire)` are pure functions of a `Double` desire, recomputed per call, and `EgoContext.invalidateCache` says in as many words that there is nothing to invalidate. There is also no direction to alias: `Side` has only `LEFT` and `RIGHT`, and `CoreTypes.kt` records the absence of a `NONE` member as a decision, naming the Java `LateralDirectionality` it refuses to copy. So the corrected behaviour is obtained by construction and nothing in the core could be keyed wrongly. Note for the port: unlike BC-11 this divergence is *undocumented* — no ADR and no `CacheMismatch`-style observer names it, although the core elsewhere preserves such quirks on purpose. |
| BC-11 — car-following cache keyed by leader **and** headway factor | **no** | The core reproduces the leader-only key on purpose. ADR-014 names "the per-tick car-following cache keyed by leader alone" among the quirks core v1 reproduces, and `RelaxedCarFollowing` implements it with the comment "which is the Java cache exactly", together with a `CacheMismatch` observer that measures the quirk rather than removing it. Its `CachedAnswer` already carries `headwayFactor` and `gapReserve`, so adopting BC-11 is a one-line key change there — but it needs ADR-014 revised first, and until then `coreset` must keep the collision or it would stop being what the core embodies. |
