...
> **Outcome: the patch was not adopted.** It adds ~5.5 percentage points on top of
> `LaneBasedGtu.CACHING=false`, which is inside noise territory for a single run per cell, and it
> carries a real operational cost — a hand-installed jar that is invisible to CI and falls back to
> stock wherever anyone forgets it. `djunits.version` stays stock `5.2.1`; this document is kept as
> the working record for the in-person Delft conversation. See
