...
* **Congestion threshold**: one **study-wide** $v_{\text{crit}}$, fitted once and shared by simulation and field, instead of a GMM refitted for every individual run.
  * The refitting was the error, not the level. Each run got its own threshold from its own ~100 intervals, which scattered between $65$ and $90\text{ km/h}$; the threshold was thereby a random variable, and a run whose fit landed high registered early crossings as breakdowns. The simulated capacity read $1544 \pm 514\text{ veh/h}$.
  * The plot now measures against both thresholds side by side, and they answer the same: over three cells the shared empirical value ($66.7$) and the simulation's own pooled GMM ($80$–$87$) give $1941$ vs $1953$, $2100$ vs $2100$, $1824$ vs $1824\text{ veh/h}$. **Which threshold is used barely matters; that it is fitted once does.** An earlier reading of this that blamed the difference between the empirical and the simulated threshold does not survive that comparison.
  * The per-dataset GMM speeds are still fitted and reported, since they describe each cloud, and the simulation's capacity under its own threshold is printed alongside so the choice remains auditable. Neither decides what counts as a breakdown.
