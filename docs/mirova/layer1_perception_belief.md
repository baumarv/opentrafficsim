...
1.  **State Properties**: Exposes core physical quantities like current Speed, Acceleration, Length, Width, Route, Target Speed, and `lastTickAcceleration` (the executed operational plan acceleration from the previous simulation step).
2.  **Deceleration Thresholds**:
