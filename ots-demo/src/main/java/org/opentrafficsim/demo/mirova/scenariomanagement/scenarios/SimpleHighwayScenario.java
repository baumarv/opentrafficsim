...
public class SimpleHighwayScenario extends ScenarioGenerator
{
    ...
    public void buildNetwork(final OtsSimulatorInterface sim) throws Exception
    {
        ...
        CrossSectionLink link = new CrossSectionLink(this.network, "AB", A, B, DefaultsNl.FREEWAY,
                new OtsLine2d(A.getPoint(), B.getPoint()), null, LaneKeepingPolicy.KEEPRIGHT);
        ...
        listLanes.add(LaneGeometryUtil.createStraightLane(link, "L1", Length.instantiateSI(-3.5), Length.instantiateSI(3.5),
                DefaultsRoadNl.FREEWAY, Map.of(DefaultsNl.VEHICLE, new Speed(250, SpeedUnit.KM_PER_HOUR))));
    ...
    {
        OdMatrix odMatrix = build3LaneDemandOD(this.network.getNode("A"), this.network.getNode("B"));
        ...
        MirovaTacticalPlannerFactory mirovaTacticalPlannerFactoryCars =
                new MirovaTacticalPlannerFactory(w99CarFactory, new DefaultMirovaPerceptionFactory())
                {
                    ...
                    public Parameters getParameters() throws ParameterException
                    {
                        ...
                        // parameters.setParameter(MirovaParameters.socialInteractionCooldown,
                        // Duration.instantiateSI(cooldown.draw()));
    ...
    public void buildRoutes()
    {
        ...
        {
            this.routes.put("A-B",
                    new Route("RouteAB", DefaultsNl.VEHICLE, List.of(this.network.getNode("A"), this.network.getNode("B"))));
        }
    ...
    public void buildRoadSamplers() throws NetworkException
    {
        ...
        RoadSampler sampler = RoadSampler.build(this.network)
                ...
                .registerExtendedDataType(new ExtendedDataSocioSpeedPressure()).create();
    ...
    public LaneBasedGtuCharacteristicsGeneratorOd buildOdsCharacteristicsGenerator(final OtsSimulatorInterface sim)
    {
        return new LaneBasedGtuCharacteristicsGeneratorOd()
        {
                    ...
                    final StreamInterface randomStream) throws GtuException
            {
                ...
                try
                {
                    route = new Route("ODRoute", gtuType, List.of(origin, destination));
                }
                catch (NetworkException exception)
                {
                    exception.printStackTrace();
                ...
                LaneBasedStrategicalPlannerFactory<?> strategical =
                        SimpleHighwayScenario.this.gtuTemplates.get(gtuType).getStrategicalPlannerFactory();
    ...
    /**
     * Builds an OD-matrix for a 3-lane motorway scenario following the demand pattern described in Weyland et al.’s simulation
     * setup. The generated demand covers a complete loading and unloading cycle, starting with a warm-up period, ramping up to
     * peak traffic, and ramping down symmetrically.
     * <p>
     * <b>Demand Profile</b>
     * <ul>
     ...
     * </ul>
     * <p>
     * <b>Matrix Structure</b><br>
     * A single OD relation (origin → destination) is populated with two demand vectors: one for cars and one for trucks. Values
     * are interpreted as step-wise interpolation (constant demand within each interval).
     * @param origin The origin node of the OD relation.
     * @param destination The destination node of the OD relation.
     * @return A fully populated {@link OdMatrix} containing the complete 3-lane demand curve with warm-up, rise, peak, and
     *         decline phases.
     * @throws Exception If the OD matrix cannot be created or demand vectors cannot be assigned.
     */

    public OdMatrix build3LaneDemandOD(final Node origin, final Node destination) throws Exception
    {
        // ---------------------------------------------------------------
        ...
        OdMatrix odMatrix = new OdMatrix("OD_3LaneScenario", getOrigins(this.network), getDestinations(this.network),
                categorization, timeVector, Interpolation.STEPWISE);
